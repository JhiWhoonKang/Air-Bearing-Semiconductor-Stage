...
namespace JKK_XYSTAGE
{
    ...
    public partial class PTP_Form : Form
    {
        ...
        int y; int x;
        List<Point> CP_point_list = new List<Point>();



        // PM Move PTP
        double anglePM =0;
        List<Point> PM_point_list = new List<Point>();    // 실제 좌표 
        List<Point> PM_point2stage_list = new List<Point>();  // twincat 보낼 좌표
        Point first_pos;  // 실제 좌표 
        Point fist_pos2Stage; 
        ...
        public PTP_Form()
        {
            ...
            if(Form1.Ads.IsConnected)
            {
                ...
                Form1.PID_Y_form.Display_PID_Gain();
            }
        ...
        private void pl_stage_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (x>=0 && x <= x_MaxPos && y>=0 && y<=y_MaxPos)
            {
                ...
                if ( !rb_CP.Checked && !rb_PM.Checked)
                {
                ...
                if ((user_point_list.Count == Move_num)&& !rb_CP.Checked && !rb_PM.Checked) //5개 찍으면 동작
                {
                ...
                else if(rb_PM.Checked)
                {
                    ...
                    Point first = new Point((int)(CircleCenter.X + Math.Cos(radian) * int.Parse(tb_PM_radius.Text))
                        , (int)(CircleCenter.Y - Math.Sin(radian) * int.Parse(tb_PM_radius.Text)));

                    PM_point_list.Add(first);
                    PM_point2stage_list.Add(new Point(Convert.ToInt32((x_MaxPos / client_stage.Width) * (client_home.X - first.X))
                          , Convert.ToInt32((y_MaxPos / client_stage.Height) * (first.Y - Margin))));
                    ...
                    while (true)
                    {
                        ...
                        if (angle >= 360) break;
                        PM_point_list.Add(new Point(posx, posy));
                        PM_point2stage_list.Add(new Point(Convert.ToInt32((x_MaxPos / client_stage.Width) * (client_home.X - posx))
                            ,Convert.ToInt32((y_MaxPos / client_stage.Height) * (posy - Margin))));


                    }
                    first_pos = PM_point_list[0];
                    fist_pos2Stage = PM_point2stage_list[0];
                    string Point_display = "[" + user_point.X.ToString() + "," + user_point.Y.ToString() + "]";
                    g.DrawString(Point_display, Font, Brushes.Blue, Mouse_Point.X - 23, Mouse_Point.Y + 12);
                    //4. user 지정 좌표 가시화 ㅁ 초록색 사각형 표시
                    g.FillRectangle(Brushes.Green, new Rectangle(Mouse_Point.X - 3, Mouse_Point.Y - 3, 6, 6));
                    ...
                    Thread.Sleep(500);
                    Thread PM_thread = new Thread(new ThreadStart(PM_Function));
                    PM_thread.Start();
                }
            }
            else 
        ...
        private void PM_Function()
        {
            while (PM_point2stage_list.Count != 0)
            {
                Point tmp = PM_point2stage_list[0];
                PM_point2stage_list.RemoveAt(0);
            ...
            Form1.Ads.WriteAny(Form1.hX_Command_Pos, Convert.ToDouble(fist_pos2Stage.X));
            Form1.Ads.WriteAny(Form1.hY_Command_Pos, Convert.ToDouble(fist_pos2Stage.Y));

            Form1.Ads.WriteAny(Form1.hX_AbMove_Ex, true);
            Form1.Ads.WriteAny(Form1.hY_AbMove_Ex, true);

            Thread.Sleep(1000);
            PM_point2stage_list.Clear();
            PM_point_list.Clear();
            InitStage();
        }
        private void CP_Function()
        {
            ...
            while (radian <= Math.PI * 2)
            {
                ...
                CP_point_list.Add(new Point(x, y));
            }

            while (CP_point_list.Count != 0)
            {
                Point tmp = CP_point_list[0];
                CP_point_list.RemoveAt(0);
                ...
                g.FillEllipse(Brushes.Black, new Rectangle(client_home.X - (int)((tmp.X * client_stage.Width) / x_MaxPos) 
                    , Margin + (int)((tmp.Y * client_stage.Height) / y_MaxPos) 
                    , 5, 5));
        ...
        private void bt_StageReset_Click(object sender, EventArgs e)
        {
            user_point_list.Clear();
            stage_point_list.Clear();
            CP_point_list.Clear();
        ...
        private void rb_PM_CheckedChanged(object sender, EventArgs e)
        {
            if (rb_PM.Checked)
            {
                tb_PM_num.ReadOnly = false;
                tb_PM_radius.ReadOnly = false;
